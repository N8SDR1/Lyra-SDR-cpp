...
; QML modules + plugins + the WDSP DLLs in _native/ + app-local MSVC
; CRT (vcruntime140.dll / msvcp140.dll via cmake/copy_msvc_crt.cmake;
; windeployqt --compiler-runtime is kept as a fallback but often no-ops
; on VS 2026 VC145)) into a single Windows installer with Start-Menu +
; optional desktop shortcuts.  Also ships LICENSE / NOTICE.md / CREDITS.md
; next to the exe (GPL attribution).
; FFTW WISDOM is generated by the app itself on first launch
; (lyra.exe --build-wisdom), so the installer needs no DSP step.
;
...
; Show the GPL v3+ text on the first wizard page (required attribution).
LicenseFile=..\LICENSE
...
Source: "{#BuildDir}\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs; \
    Excludes: "test_*.exe,*.lib,*.pdb,*.exp,*.ilk,*.cmake,*.ninja,CMakeCache.txt,.ninja_deps,.ninja_log,CMakeFiles\*,.qt\*,.qsb\*,.rcc\*,lyra_autogen\*,meta_types\*,qmltooling\*,qmltypes\*,third_party\*"
...
; GPL attribution files (not produced by windeployqt).
